...
Public Sub AddSystray(myForm As Form, myTip As String)
    With nID
        ...
        .hIcon = myForm.Icon            'icon on the mainform
...
Public Sub ModifySystray(myForm As Form, myTip As String)
    With nID
        ...
        .uFlags = NIF_ICON Or NIF_TIP Or NIF_MESSAGE
        .uCallBackMessage = WM_MOUSEMOVE
        .hIcon = myForm.Icon                'icon on the mainform, to change systray icon, first change the mainform icon then call ModifySystray
        .szTip = myTip & vbNullChar
    End With
